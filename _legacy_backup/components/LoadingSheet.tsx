                                        ...
                                        return Array.from({ length: rowsNeeded }).map((_, rIndex) => (
                                            <tr key={`${lItem.skuSrNo}-${rIndex}`} className={`hover:bg-blue-50/20 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                                ...
                                                {Array.from({ length: 10 }).map((_, cIndex) => {
                                                    ...
                                                    return <td key={cIndex} className={`border-r border-b border-slate-100 p-0 text-center w-10 h-10 ${!isValid ? 'bg-slate-50' : ''}`}>{isValid && <input type="number" className="w-full h-full text-center outline-none bg-transparent focus:bg-blue-50" value={cell?.value !== undefined && cell.value !== 0 ? cell.value : ''} onChange={e => handleLoadingCellChange(lItem.skuSrNo, rIndex, cIndex, e.target.value)} onFocus={(e) => e.target.select()} onBlur={(e) => handleCellBlur(lItem.skuSrNo, rIndex, cIndex, e.target.value)} disabled={isCompleted} placeholder="" />}</td>
                                                })}
                                                {rIndex === 0 && <><td rowSpan={rowsNeeded} className="border-r border-b border-slate-100 p-0"><input type="number" placeholder="" className="w-full h-full text-center outline-none bg-transparent focus:bg-blue-50" value={lItem.looseInput !== undefined && lItem.looseInput !== 0 ? lItem.looseInput : ''} onChange={e => handleLooseChange(lItem.skuSrNo, e.target.value)} onFocus={(e) => e.target.select()} disabled={isCompleted} /></td><td rowSpan={rowsNeeded} className="border-r border-b border-slate-100 p-3 text-center font-bold">{lItem.total}</td><td rowSpan={rowsNeeded} className={`border-b border-slate-100 p-3 text-center font-bold ${lItem.balance !== 0 ? 'text-red-600' : 'text-green-600'}`}>{lItem.balance}</td></>}
                                            </tr>
                                        ));
                                    })}
                                </tbody>
